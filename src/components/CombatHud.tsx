...
import { Shield, BookOpen, ChevronDown, ChevronRight, Users, User, Sparkles } from 'lucide-react';
        ...
        {activeNpcs.length > 0 && (
          <div>
            ...
            <div className="flex flex-col gap-1">
              {activeNpcs.map(npc => (
                <div key={npc.id} className="flex items-center gap-1.5" title={npc.relation || 'PNJ'}>
      ...
      <div className="px-3 py-1.5 flex justify-between items-center bg-black/30 backdrop-blur-xs">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="text-[11px] uppercase tracking-wider">Estado del personaje</span>
          </button>
          ...
          {!isExpanded && (
            <div className="flex items-center gap-2 text-[11px] ml-2">
              ...
              <span className={`px-1.5 py-0.2 rounded font-bold ${getHpColor()}`}>
                HP {currentHp}/{maxHp}
              </span>
        ...
        <div className="p-2.5 md:p-3 grid grid-cols-1 md:grid-cols-3 gap-3 items-center bg-gradient-to-r from-[#221612] via-[#2d201c] to-[#221612]">
          ...
          <div className="flex items-center gap-3 bg-black/25 p-2 rounded-lg border border-[var(--light-gold)]/20">
            ...
            >
              ...
              <div className="text-xs font-bold text-[var(--light-gold)]">{ac}</div>
            </div>
          </div>
          ...
          <div className="bg-black/25 p-2 rounded-lg border border-[var(--light-gold)]/20 flex flex-col justify-between h-full">
            <div className="flex justify-between items-center text-[10px] text-amber-300/90 font-cinzel mb-0.5">
              <span className="flex items-center gap-1">
                <Sparkles className="w-3 h-3 text-amber-300 inline" /> ESTADO ACTUAL
              </span>
              <span className="text-white/60">Escena</span>
            </div>
            <p className="text-[11px] text-[#f4ecd8]/90 line-clamp-2 m-0 italic">
              "{project.memory.current_status || 'Explorando y atentos a los movimientos del entorno.'}"
            </p>
          </div>
          ...
          <div className="bg-black/25 p-2 rounded-lg border border-[var(--light-gold)]/20 flex flex-col justify-between h-full">
            <div className="flex justify-between items-center text-[10px] text-amber-300/90 font-cinzel mb-1">
              <span className="flex items-center gap-1">
                <Users className="w-3 h-3 text-amber-300 inline" /> COMPAÑEROS & AFINIDAD
              </span>
              <span className="text-[9px] text-white/50">{activeNpcs.length} activos</span>
            </div>

            {activeNpcs.length === 0 ? (
              <div className="text-[10px] text-white/50 italic">Sin acompañantes registrados en memoria.</div>
            ) : (
                  ...
                  >
                    ...
                    {(npc.atr !== undefined || npc.vin !== undefined || npc.con !== undefined) && (
                      <div className="flex items-center gap-1 text-[8px] font-mono font-bold pl-0.5 border-l border-white/15">
                        <span className="text-rose-400" title={`Atracción: ${npc.atr ?? 0}/10`}>♥{npc.atr ?? 0}</span>
                        <span className="text-teal-400" title={`Vínculo: ${npc.vin ?? 0}/10`}>★{npc.vin ?? 0}</span>
                        <span className="text-amber-400" title={`Confianza: ${npc.con ?? 0}/10`}>♦{npc.con ?? 0}</span>
                      </div>
                    )}
                  </div>
                ))}
