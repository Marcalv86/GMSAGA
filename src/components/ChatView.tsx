...
interface ChatMessageItemProps {
  ...
  setDeleteModal: (
    v: { index: number; role: "user" | "model"; isLast: boolean } | null,
  ) => void;
...
const ChatMessageItem = React.memo<ChatMessageItemProps>(
  ({
    ...
    const invitaciones =
      isModel && hasOracle ? leerInvitaciones(m.content) : [];
    const baseContent = isModel
      ? limpiarInvitaciones(
          stripStateTag(
            rollRequests.length ? stripRollRequests(m.content) : m.content,
          ),
        )
      : m.content;
    ...
    const bodyText = isModel
      ? formatNarrativeText(narrativeText)
      : narrativeText;
    ...
    if (isModel) {
      ...
      if (
        ...
        syncItems.push("Afinidad PNJs");
      if (/\[\s*PRESENTES\s*:/i.test(m.content))
        syncItems.push("PNJs en escena");
    }
      ...
      >
        ...
        {isEditing ? (
          <div className="w-full max-w-[900px] bg-[var(--surface)] border-2 border-[var(--accent)] rounded-lg p-3 shadow-md">
            <div className="text-xs font-cinzel font-bold text-[var(--accent)] mb-2 flex justify-between items-center">
              <span>
                ...
                Editando{" "}
                {m.role === "user"
                  ? "tu mensaje de acción"
                  : "la respuesta del Narrador"}
              </span>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 mt-3 pt-2 border-t border-[var(--glass-border)]">
              ...
              <div className="flex gap-2">
                ...
                {m.role === "user" ? (
                  ...
                  >
                    <RefreshCw className="w-3.5 h-3.5" /> Guardar y Regenerar
                    Trama
                  </button>
                ) : (
                  <button
          ...
          >
              ...
              <div className="markdown-body narrative-body">
                <ReactMarkdown
                  components={{
                    p: ({ children }) => {
                      const str = Array.isArray(children)
                        ...
                        : typeof children === "string"
                          ...
                          : "";
                      const isDialogue =
                        isModel && /^[—–\-"«]/.test(str.trim());
                      return (
                        <p
                          className={
                            isDialogue ? "narrative-dialogue" : undefined
                          }
                        >
                          {children}
                        </p>
                    ...
                    strong: ({ children }) => (
                      <strong className="narrative-strong">{children}</strong>
                    ),
                    em: ({ children }) => (
            ...
            {rolls.length > 0 && (
              <div
                className={`space-y-1.5 ${bodyText ? "mt-2.5 pt-2 border-t border-[var(--glass-border)]/60" : ""}`}
              >
                {rolls.map((r, rIdx) => (
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {req.dc && (
                              <span
                                className={`text-[11px] font-cinzel font-bold px-2 py-0.5 rounded-full border shadow-2xs flex items-center gap-1 ${dcBadgeClass}`}
                              >
                                <span>CD {req.dc}</span>
...
export const ChatView: React.FC<{
  ...
  const [probabilidad, setProbabilidad] = useState<Probabilidad>(
    PROBABILIDAD_POR_DEFECTO,
  );
  ...
  const hits = React.useMemo(() => {
    ...
    return chat.messages
      .map((m, i) => (normalise(m.content || "").includes(q) ? i : -1))
      .filter((i) => i >= 0);
  ...
  const toggleSpeechRecognition = () => {
    ...
    if (!SpeechRec) {
      alert(
        "Tu navegador no soporta reconocimiento de voz nativo en esta ventana. Por favor prueba con Google Chrome, Edge o Safari.",
      );
      return;
    }
    ...
    try {
      ...
      rec.onerror = (event: any) => {
        ...
        if (event.error === "not-allowed") {
          alert(
            "Permiso de micrófono denegado. Permite el acceso al micrófono en el navegador para dictar tus acciones de rol.",
          );
        }
        setIsListening(false);
      };
      ...
      {isDraggingOver && (
        <div className="absolute inset-0 bg-amber-900/60 backdrop-blur-xs z-50 flex flex-col items-center justify-center text-white border-4 border-dashed border-amber-300 pointer-events-none animate-[fadeIn_0.15s_ease]">
          ...
          <p className="text-sm opacity-90">
            Se procesarán todos los archivos juntos y estarán disponibles para
            el Narrador.
          </p>
        </div>
      )}
      ...
      <div className="bg-[color-mix(in_srgb,var(--surface)_90%,transparent)] border-b border-[var(--glass-border)] px-3 sm:px-4 md:px-6 py-2.5 flex justify-between items-center gap-2 md:gap-3 shadow-2xs shrink-0 z-10">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          {isSearchOpen ? (
            <div className="flex items-center gap-1.5 bg-[var(--bg-color)] border border-[var(--user-border)] rounded-lg px-2 py-1 shadow-2xs">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter")
                    goToHit(e.shiftKey ? activeHit - 1 : activeHit + 1);
                  if (e.key === "Escape") {
          ...
          >
            ...
            <span className="hidden sm:inline">Exportar Tomo (PDF)</span>
          </button>
        </div>
      </div>
            ...
            {!chat?.messages || chat.messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center text-center py-6 sm:py-10 px-4 max-w-md mx-auto">
                ...
                <p className="text-xs sm:text-sm text-[var(--text-secondary)] italic font-lora leading-relaxed">
                  Escribe la primera acción de tu personaje o lanza los dados
                  para que el Narrador comience el relato.
                </p>
              </div>
            ) : (
      ...
      <div className="px-2.5 sm:px-4 md:px-8 pt-2.5 pb-4 md:py-5 border-t border-dashed border-[var(--glass-border)] bg-gradient-to-t from-[color-mix(in_srgb,var(--sidebar-bg)_95%,transparent)] to-transparent">
        ...
        <div className="max-w-[900px] mx-auto mb-2 flex flex-col items-center gap-2">
          {activeRoll && (
            <div className="animate-[bounce_1s_infinite] bg-gradient-to-r from-[var(--surface)] via-[var(--bg-color)] to-[var(--surface)] text-[var(--text-primary)] px-4 py-2 rounded-2xl shadow-xl shadow-black/25 border-2 border-[var(--accent)] flex items-center gap-3 z-30">
              <div
                className={`w-8 h-8 rounded-xl flex items-center justify-center font-cinzel font-bold text-xs sm:text-sm shadow-md border ${
              ...
              <div className="flex flex-col">
                <div className="flex items-center gap-2">
                  <span className="font-cinzel text-xs font-semibold text-[var(--text-secondary)]">
                    Resultado:
                  </span>
                  <span
                    className={`font-cinzel text-lg sm:text-xl font-black ${
                      activeRoll.result === 20 && activeRoll.sides === 20
                ...
                <span className="text-[10px] text-[var(--text-secondary)] font-lora italic">
                  Añadido automáticamente a tu mensaje
                </span>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-1 sm:gap-1.5 md:gap-2 justify-center items-center flex-wrap">
            ...
            >
              ...
              <span className="hidden sm:inline">Bardo & Música</span>
            </button>
        ...
        <div className="max-w-[900px] mx-auto bg-[var(--bg-color)] border border-[var(--user-border)] rounded-xl px-2.5 sm:px-3.5 md:px-4 py-1.5 md:py-2 flex items-center gap-1.5 sm:gap-2 md:gap-3 shadow-inner focus-within:border-[var(--accent)] focus-within:shadow-md transition-all">
          ...
          <button
            ...
            title={
              isListening
                ...
                : "Dictar tu acción por voz (Micrófono en vivo)"
            }
          >
            {isListening ? (
          ...
          <textarea
            ...
            placeholder={
              isListening
                ? "🎙️ Escuchando... habla con normalidad..."
                : "¿Qué hace tu personaje? (Escribe o pulsa el micrófono para dictar)"
            }
            ...
            <div className="flex flex-col gap-2.5 mb-5">
              ...
              >
                ...
                <div className="text-[11px] text-[var(--text-secondary)]">
                  Elimina exclusivamente este turno sin tocar los mensajes
              ...
              {!deleteModal.isLast && (
                ...
                >
                  ...
                  <div className="text-[11px] text-red-700">
                    Borra este mensaje{" "}
                    <strong>y todos los mensajes posteriores</strong>,
                    permitiéndote re-tomar la historia desde este punto exacto.
                  </div>
                </button>
              )}
            </div>
