...
import { PlayerCharacter, InventoryItem, CharacterSpell, CharacterTrait } from '../types';
import { ensureValidPlayerCharacter, DND_SKILL_DEFINITIONS, calculateModifier, formatModifier } from '../utils/characterSheetParser';
import {
  Shield,
  Swords,
  Heart,
...
interface CharacterSheetViewProps {
  ...
  onUpdateCharacter?: (updater: (prevPc: PlayerCharacter) => PlayerCharacter) => Promise<void>;
...
export const CharacterSheetView: React.FC<CharacterSheetViewProps> = ({
  ...
  const character = useMemo(() => ensureValidPlayerCharacter(rawCharacter), [rawCharacter]);
  const [activeTab, setActiveTab] = useState<'combat' | 'magic' | 'inventory' | 'profile'>('combat');
  ...
  const getSavingThrowBonus = (statKey: string, score: number) => {
    const isProf = savingThrowProfs.some(p => p.toUpperCase() === statKey.toUpperCase());
  ...
  const statList = [
    ...
    { key: 'con', name: 'Constitución', score: attributes.con || 10, short: 'CON' },
    { key: 'int', name: 'Inteligencia', score: attributes.int || 10, short: 'INT' },
    { key: 'wis', name: 'Sabiduría', score: attributes.wis || 10, short: 'SAB' },
    { key: 'cha', name: 'Carisma', score: attributes.cha || 10, short: 'CAR' }
  ];

  // Dynamic Skill calculations
  const computedSkills = useMemo(() => {
    const profSkillsLower = (character.skillProficiencies || []).map(s => s.toLowerCase().trim());

    return DND_SKILL_DEFINITIONS.map(def => {
      const statScore = attributes[def.statKey] || 10;
      const baseMod = calculateModifier(statScore);

      const isExpert = profSkillsLower.some(p =>
        (p.includes('pericia') || p.includes('experta') || p.includes('expert')) &&
        def.aliases.some(alias => p.includes(alias))
      );

      const isProf =
        isExpert ||
        profSkillsLower.some(p => def.aliases.some(alias => p.includes(alias)));
  ...
  const resolvedTraits = useMemo<CharacterTrait[]>(() => {
    ...
    for (const b of blocks) {
      ...
      const titleMatch = clean.match(/^(?:###\s*|\*\*\s*|[•\-\*]\s+\*\*)([^*:\n#]+)(?:\*\*|:)?\s*([\s\S]*)$/);
      if (titleMatch) {
        const title = titleMatch[1].replace(/[*#]/g, '').trim();
        const desc = titleMatch[2].replace(/^[:\s-]+/, '').trim();
        if (title.length > 2) {
          const isFeat = /dote|feat/i.test(title) || /afortunado|iniciado|alerta|tirador|robusto/i.test(title);
          traits.push({
            name: title,
  ...
  const resolvedSpells = useMemo<CharacterSpell[]>(() => {
    ...
    const spellMatches = sheetText.match(/(?:Trucos|Conjuros|Spells|Cantrips)[\s\S]*?(?:###|\n\n\n|$)/i);
    if (spellMatches) {
        ...
        if (itemMatch) {
          ...
          if (name && !/trucos|conjuros|nivel/i.test(name)) {
            spells.push({
              name,
              level: currentLevel,
              castingTime: itemMatch[2]?.includes('acción') ? itemMatch[2] : '1 acción',
              range: itemMatch[2]?.includes('pies') || itemMatch[2]?.includes('toque') ? itemMatch[2] : undefined,
              description: itemMatch[3] || itemMatch[2] || 'Conjuro preparado según la crónica y trasfondo.',
  ...
  const hasSpellcasting = Boolean(
    ...
    (character.class && /druida|mago|brujo|cl[eé]rigo|hechicero|bardo|palad[ií]n|explorador|art[ií]fice|wizard|sorcerer|warlock|cleric|druid|bard|paladin|ranger/i.test(character.class))
  );
  ...
  const maxCarryWeight = character.maxCarryWeight || (attributes.str || 10) * 15;
  const carryPercent = Math.min(100, Math.round((totalWeight / Math.max(1, maxCarryWeight)) * 100));
  ...
  const hpPercent = Math.max(0, Math.min(100, Math.round((hp / Math.max(1, maxHp)) * 100)));
  ...
  const parsedLanguages = useMemo(() => {
    ...
    const found: string[] = [];
    ['Común', 'Drúidico', 'Silvano', 'Élfico', 'Enano', 'Orco', 'Gnómico', 'Mediano', 'Dracónico', 'Infernal', 'Abisal', 'Celestial', 'Primordial'].forEach(lang => {
      if (new RegExp(`\\b${lang}\\b`, 'i').test(txt)) {
        found.push(lang);
      }
    });
    return found.length ? found : ['Común'];
  }, [character.languages, character.proficienciesAndLanguages, character.sheetText]);
  ...
  return (
    <div className="space-y-6 animate-[fadeIn_0.2s_ease]">
      ...
      <div className="relative rounded-3xl overflow-hidden border-2 border-amber-500/40 bg-gradient-to-b from-amber-950/40 via-stone-900/90 to-black/95 text-amber-100 p-6 md:p-8 shadow-xl">
          ...
          <div className="flex-1 text-center md:text-left space-y-2">
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2.5">
              ...
              {character.level && (
                <span className="text-xs font-cinzel font-bold px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/40">
                  {character.level.startsWith('Nivel') ? character.level : `Nivel ${character.level}`}
                </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-4 gap-y-1 text-xs sm:text-sm text-amber-200/80 font-cinzel">
              ...
              {character.alignment && (
                <span className="flex items-center gap-1">
                  <span>·</span>
                  <span className="italic opacity-80">{character.alignment}</span>
                </span>
              )}
            </div>
        ...
        <div className="mt-6 pt-6 border-t border-amber-500/20 grid grid-cols-2 sm:grid-cols-5 gap-3">
          ...
          <div className="col-span-2 sm:col-span-2 bg-black/40 rounded-2xl p-3.5 border border-amber-500/30 flex flex-col justify-between shadow-inner">
            <div className="flex justify-between items-center text-xs font-cinzel">
              <span className="font-bold text-amber-300 flex items-center gap-1.5 uppercase">
                <Heart className="w-4 h-4 text-rose-500 fill-rose-500/20" /> Puntos de Golpe (PG)
              </span>
            ...
            <div className="flex items-center justify-between text-[11px] text-amber-200/70 font-cinzel">
              <span>{hp <= 0 ? '💀 Inconsciente' : hpPercent <= 30 ? '⚠️ Herido' : '🛡️ Vitalidad activa'}</span>
              {character.conditions && character.conditions.length > 0 && (
                <span className="text-rose-400 font-bold">{character.conditions.join(', ')}</span>
              )}
            </div>
          </div>
          ...
          <div className="bg-black/40 rounded-2xl p-3 border border-amber-500/30 flex flex-col items-center justify-center text-center shadow-inner">
            ...
            <span className="text-[10px] text-amber-300/60 font-cinzel">CA Defensiva</span>
          </div>
          ...
          <div className="bg-black/40 rounded-2xl p-3 border border-amber-500/30 flex flex-col items-center justify-center text-center shadow-inner">
            ...
            <span className="text-[10px] text-amber-300/60 font-cinzel">Desplazamiento</span>
          </div>
          ...
          <div className="bg-black/40 rounded-2xl p-3 border border-amber-500/30 flex flex-col items-center justify-center text-center shadow-inner">
            ...
            <div className="flex items-center gap-2 my-0.5">
              <span className="font-cinzel text-lg font-bold text-amber-200">{initiative}</span>
              <span className="text-amber-500">·</span>
              <span className="font-cinzel text-lg font-bold text-amber-400">+{proficiencyBonus}</span>
            </div>
            <span className="text-[10px] text-amber-300/60 font-cinzel">Iniciativa · Bono</span>
          </div>
        </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-center gap-1 w-full">
                    ...
                    {isProf && (
                      <span className="text-[9px] text-amber-400 font-bold" title="Competencia en Salvación">
                        ★
                      </span>
                    )}
                  </div>
                  <span className="font-cinzel text-xl font-black text-amber-100 my-0.5">
                    {st.score} <span className="text-xs text-amber-400/90 font-bold">({mod})</span>
                  </span>
                  <div className="text-[9px] font-cinzel text-amber-300/70 border-t border-amber-500/20 w-full pt-1 mt-0.5 flex items-center justify-center gap-1">
                    <span>Salv:</span>
                    <span className={`font-bold ${isProf ? 'text-amber-300' : 'text-amber-200/60'}`}>
                      {bonus}
                    </span>
                  </div>
                </div>
      ...
      <div className="flex items-center justify-center sm:justify-start border-b border-[var(--glass-border)] gap-2 flex-wrap pb-1">
        {[
          { id: 'combat', label: '⚔️ Habilidades & Rasgos', count: resolvedTraits.length ? `(${resolvedTraits.length})` : '' },
          { id: 'magic', label: '🔮 Magia & Conjuros', count: resolvedSpells.length ? `(${resolvedSpells.length})` : (hasSpellcasting ? '' : '•') },
          { id: 'inventory', label: '🎒 Inventario & Equipo', count: inventory.length ? `(${inventory.length})` : '' },
          ...
          >
            <span>{tab.label}</span>
            {tab.count && <span className="text-[11px] opacity-80">{tab.count}</span>}
          </button>
        ))}
      </div>
              ...
              {computedSkills.map(sk => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`font-cinzel font-medium ${sk.isProf || sk.isExpert ? 'font-bold text-[var(--text-primary)]' : ''}`}>
                      {sk.name}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`font-cinzel font-black text-sm ${sk.isExpert ? 'text-purple-600 dark:text-purple-400 font-extrabold' : sk.isProf ? 'text-amber-600 dark:text-amber-400' : 'text-[var(--text-secondary)]'}`}>
                      {sk.bonusStr}
                      {sk.passivePerception ? ` (Pasiva ${sk.passivePerception})` : ''}
                    </span>
                  </div>
                </div>
              ))}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-cinzel text-base font-bold text-[var(--accent)] flex items-center gap-2 m-0">
                <Flame className="w-5 h-5 text-orange-500" /> Rasgos de Clase, Dotes & Aptitudes
              </h3>
            ...
            {resolvedTraits.length > 0 ? (
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2">
                          <div className="p-1.5 rounded-lg bg-[var(--surface)] border border-[var(--glass-border)] text-amber-500">
                            {t.type === 'feat' ? <Star className="w-4 h-4" /> : <Flame className="w-4 h-4" />}
                          </div>
                          <div>
                            ...
                            <span className="text-[10px] font-cinzel text-[var(--accent)] font-semibold">
                              {t.source || (t.type === 'feat' ? 'Dote Especial' : 'Rasgo de Clase')}
                            </span>
                          </div>
                        </div>
              ...
              <div className="p-6 bg-[var(--surface-soft)] rounded-2xl border border-[var(--user-border)] text-center text-xs text-[var(--text-secondary)] font-lora">
                No se registraron rasgos de clase o dotes adicionales en el documento.
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-[var(--surface-soft)] border border-[var(--user-border)] p-4 rounded-2xl shadow-sm space-y-3">
              <h4 className="font-cinzel text-xs font-bold text-[var(--accent)] flex items-center gap-1.5 m-0 uppercase">
                <Languages className="w-4 h-4 text-emerald-500" /> Idiomas Conocidos
              </h4>
            ...
            <div className="bg-[var(--surface-soft)] border border-[var(--user-border)] p-4 rounded-2xl shadow-sm space-y-3">
              <h4 className="font-cinzel text-xs font-bold text-[var(--accent)] flex items-center gap-1.5 m-0 uppercase">
                <Wrench className="w-4 h-4 text-amber-500" /> Competencias en Equipo & Herramientas
              </h4>
              <p className="text-xs text-[var(--text-primary)] m-0 font-lora leading-relaxed">
                {character.proficienciesAndLanguages || 'Armaduras ligeras, bastones, lanzas, herramientas de herboristería y focos druídicos.'}
              </p>
            </div>
          </div>
        </div>
          ...
          {hasSpellcasting ? (
              ...
              <div className="bg-[var(--surface-soft)] border border-[var(--user-border)] p-5 rounded-2xl shadow-sm space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-center font-cinzel">
                  <div className="p-3.5 bg-[var(--surface)] rounded-xl border border-[var(--glass-border)]">
                    <span className="text-[10px] text-[var(--text-secondary)] uppercase block">Característica Mágica</span>
                    <span className="text-base font-bold text-[var(--accent)]">
                      {character.spellcasting?.ability || 'Sabiduría'}
                    </span>
                  </div>
                  <div className="p-3.5 bg-[var(--surface)] rounded-xl border border-[var(--glass-border)]">
                    <span className="text-[10px] text-[var(--text-secondary)] uppercase block">CD Salvación Conjuros</span>
                    <span className="text-xl font-black text-[var(--accent)]">
                      {character.spellcasting?.saveDc || (8 + proficiencyBonus + calculateModifier(attributes.wis))}
                    </span>
                  </div>
                  <div className="p-3.5 bg-[var(--surface)] rounded-xl border border-[var(--glass-border)]">
                    <span className="text-[10px] text-[var(--text-secondary)] uppercase block">Bono Ataque Conjuros</span>
                    <span className="text-xl font-black text-[var(--accent)]">
                      {character.spellcasting?.attackBonus !== undefined
                        ? formatModifier(character.spellcasting.attackBonus)
                        : formatModifier(proficiencyBonus + calculateModifier(attributes.wis))}
                    </span>
                  </div>
                </div>
              </div>
                    ...
                    >
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-3 gap-1.5 text-[10px] font-cinzel bg-[var(--surface)] p-2 rounded-xl border border-[var(--glass-border)]">
                          <div>
                            <span className="text-[var(--text-secondary)] block">Tiempo:</span>
                            <span className="font-bold text-[var(--text-primary)]">{sp.castingTime || '1 acción'}</span>
                          </div>
                          <div>
                            <span className="text-[var(--text-secondary)] block">Alcance:</span>
                            <span className="font-bold text-[var(--text-primary)]">{sp.range || 'Toque/30 pies'}</span>
                          </div>
                          <div>
                            <span className="text-[var(--text-secondary)] block">Duración:</span>
                            <span className="font-bold text-[var(--text-primary)]">{sp.duration || 'Instantáneo'}</span>
                          </div>
                        </div>
            ...
            <div className="py-12 px-6 text-center bg-[var(--surface-soft)] rounded-3xl border border-[var(--user-border)] space-y-3">
              ...
              <p className="text-xs text-[var(--text-secondary)] max-w-md mx-auto font-lora m-0">
                Este personaje no posee aptitud mágica registrada ni ranuras de conjuro en su ficha. Su potencial reside en el combate marcial, habilidades físicas y técnicas tácticas.
              </p>
            </div>
          )}
        </div>
            ...
            <div className="md:col-span-2 bg-[var(--surface-soft)] border border-[var(--user-border)] p-4 rounded-2xl shadow-sm space-y-3">
              <div className="flex justify-between items-center">
                <span className="font-cinzel text-xs font-bold text-[var(--accent)] flex items-center gap-1.5">
                  <Coins className="w-4 h-4 text-amber-500" /> Monedero de Campaña
                </span>
                <span className="text-xs font-cinzel text-[var(--text-secondary)]">
                  Total estimado: <strong className="text-[var(--text-primary)]">{totalGoldValue} PO</strong>
                </span>
              </div>
              ...
              <div className="grid grid-cols-5 gap-2 text-center font-cinzel">
                <div className="bg-[var(--surface)] border border-amber-500/40 p-2 rounded-xl">
                  <span className="text-[10px] text-amber-600 dark:text-amber-400 font-bold block">ORO (PO)</span>
                  <span className="text-base font-black text-[var(--text-primary)]">{currencies.gp ?? 0}</span>
                </div>
                <div className="bg-[var(--surface)] border border-slate-400/40 p-2 rounded-xl">
                  <span className="text-[10px] text-slate-500 dark:text-slate-300 font-bold block">PLATA (PP)</span>
                  <span className="text-base font-black text-[var(--text-primary)]">{currencies.sp ?? 0}</span>
                </div>
                <div className="bg-[var(--surface)] border border-orange-600/40 p-2 rounded-xl">
                  <span className="text-[10px] text-orange-600 dark:text-orange-400 font-bold block">COBRE (PC)</span>
                  <span className="text-base font-black text-[var(--text-primary)]">{currencies.cp ?? 0}</span>
                </div>
                <div className="bg-[var(--surface)] border border-cyan-600/40 p-2 rounded-xl">
                  <span className="text-[10px] text-cyan-600 dark:text-cyan-400 font-bold block">ELECTRO (PE)</span>
                  <span className="text-base font-black text-[var(--text-primary)]">{currencies.ep ?? 0}</span>
                </div>
                <div className="bg-[var(--surface)] border border-purple-500/40 p-2 rounded-xl">
                  <span className="text-[10px] text-purple-600 dark:text-purple-400 font-bold block">PLATINO (PT)</span>
                  <span className="text-base font-black text-[var(--text-primary)]">{currencies.pp ?? 0}</span>
                </div>
              </div>
            </div>
              ...
              <div className="w-full bg-black/10 dark:bg-black/30 rounded-full h-2 overflow-hidden border border-[var(--user-border)]">
                <div
                  className={`h-full rounded-full transition-all ${
                    carryPercent > 90 ? 'bg-red-500' : carryPercent > 70 ? 'bg-amber-500' : 'bg-[var(--accent)]'
                  }`}
                  style={{ width: `${carryPercent}%` }}
                />
              </div>
              <p className="text-[11px] text-[var(--text-secondary)] m-0 leading-tight italic">
                {carryPercent > 100 ? '⚠️ Sobrecargado' : 'Mochila equilibrada para marcha y combate.'}
              </p>
            </div>
          </div>
              ...
              {filteredInventory.map(item => (
                <div
                  ...
                  className={`group p-3.5 rounded-2xl border transition-all cursor-pointer bg-[var(--surface-soft)] hover:border-[var(--accent)] hover:shadow-md flex flex-col justify-between gap-2 relative overflow-hidden ${
                    item.equipped ? 'border-amber-500/50 bg-amber-500/5' : 'border-[var(--user-border)]'
                  }`}
                >
                  <div className="space-y-1.5">
                    <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-[var(--glass-border)] text-[10px] text-[var(--text-secondary)] font-cinzel">
                    <span>{item.weight ? `${item.weight} lbs` : 'Peso ligero'}</span>
                    <span className="text-[var(--accent)] group-hover:underline">Ver detalles ❖</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-[var(--surface-soft)] border border-[var(--user-border)] p-5 rounded-2xl shadow-sm space-y-2.5">
              <h4 className="font-cinzel text-xs font-bold text-[var(--accent)] flex items-center gap-1.5 m-0 uppercase">
                <Eye className="w-4 h-4 text-amber-500" /> Apariencia Física & Porte
              </h4>
              <p className="text-xs leading-relaxed text-[var(--text-primary)] m-0 font-lora bg-[var(--surface)] p-3.5 rounded-xl border border-[var(--glass-border)]">
                {character.appearance || 'Rasgos físicos, complexión, mirada y vestimenta descritos en la crónica del personaje.'}
              </p>
            </div>
            ...
            <div className="bg-[var(--surface-soft)] border border-[var(--user-border)] p-5 rounded-2xl shadow-sm space-y-2.5">
              <h4 className="font-cinzel text-xs font-bold text-[var(--accent)] flex items-center gap-1.5 m-0 uppercase">
                <Sparkles className="w-4 h-4 text-purple-500" /> Personalidad & Actitud
              </h4>
              <p className="text-xs leading-relaxed text-[var(--text-primary)] m-0 font-lora bg-[var(--surface)] p-3.5 rounded-xl border border-[var(--glass-border)]">
                {character.personality || 'Comportamiento habitual, serenidad, templanza o instinto ante el peligro.'}
              </p>
            </div>
            ...
            <div className="bg-[var(--surface-soft)] border border-[var(--user-border)] p-5 rounded-2xl shadow-sm space-y-2.5">
              <h4 className="font-cinzel text-xs font-bold text-[var(--accent)] flex items-center gap-1.5 m-0 uppercase">
                <Star className="w-4 h-4 text-amber-400" /> Ideales & Principios Éticos
              </h4>
              <p className="text-xs leading-relaxed text-[var(--text-primary)] m-0 font-lora bg-[var(--surface)] p-3.5 rounded-xl border border-[var(--glass-border)]">
                {character.ideals || 'Principios que guían sus decisiones, respeto a la naturaleza y al equilibrio cósmico.'}
              </p>
            </div>
            ...
            <div className="bg-[var(--surface-soft)] border border-[var(--user-border)] p-5 rounded-2xl shadow-sm space-y-2.5">
              ...
              <p className="text-xs leading-relaxed text-[var(--text-primary)] m-0 font-lora bg-[var(--surface)] p-3.5 rounded-xl border border-[var(--glass-border)]">
                {character.bonds || 'Lazos con su arboleda, compañeros, protectores del círculo o juramentos del pasado.'}
              </p>
            </div>
            ...
            <div className="bg-[var(--surface-soft)] border border-[var(--user-border)] p-5 rounded-2xl shadow-sm space-y-2.5">
              <h4 className="font-cinzel text-xs font-bold text-[var(--accent)] flex items-center gap-1.5 m-0 uppercase">
                <AlertTriangle className="w-4 h-4 text-orange-500" /> Defectos & Sombras
              </h4>
              <p className="text-xs leading-relaxed text-[var(--text-primary)] m-0 font-lora bg-[var(--surface)] p-3.5 rounded-xl border border-[var(--glass-border)]">
                {character.flaws || 'Dudas, vulnerabilidades emocionales o secretos que acechan su sendero.'}
              </p>
            </div>
            ...
            <div className="bg-[var(--surface-soft)] border border-[var(--user-border)] p-5 rounded-2xl shadow-sm space-y-2.5">
              <h4 className="font-cinzel text-xs font-bold text-[var(--accent)] flex items-center gap-1.5 m-0 uppercase">
                <Feather className="w-4 h-4 text-amber-500" /> Historia de Origen & Trasfondo
              </h4>
              <p className="text-xs leading-relaxed text-[var(--text-primary)] m-0 font-lora bg-[var(--surface)] p-3.5 rounded-xl border border-[var(--glass-border)]">
                {character.backstory || 'El origen del protagonista, su linaje y las razones que lo llevaron al camino de la aventura.'}
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="p-4 space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-2 text-[11px] font-cinzel">
                <div className="p-2 bg-[var(--surface-soft)] rounded-lg border border-[var(--user-border)]">
                  <span className="text-[var(--text-secondary)] block">Cantidad:</span>
                  <span className="font-bold text-[var(--text-primary)]">{inspectItem.quantity || 1}</span>
                </div>
                <div className="p-2 bg-[var(--surface-soft)] rounded-lg border border-[var(--user-border)]">
                  <span className="text-[var(--text-secondary)] block">Peso:</span>
                  <span className="font-bold text-[var(--text-primary)]">
              ...
              <div className="space-y-1 pt-1">
                ...
                <p className="text-[var(--text-primary)] leading-relaxed m-0 font-lora bg-[var(--surface)] p-3 rounded-xl border border-[var(--user-border)]">
                  {inspectItem.description || 'Objeto perteneciente al inventario del protagonista.'}
                </p>
              </div>
            </div>
