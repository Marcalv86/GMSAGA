...
export const StatusView: React.FC<StatusViewProps> = ({
  ...
  const toggleQuest = (id: string) => {
    ...
    onUpdateMemory(prev => {
      const mem = prev || { story: '', quests: [], npcs: [], locations: [], current_status: '' };
        ...
        <div className="bg-[var(--surface-soft)] border border-[var(--glass-border)] p-4 rounded-xl flex items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="font-cinzel font-bold text-xs sm:text-sm text-[var(--text-primary)] flex items-center gap-2">
              <span>Tiradas de PNJs y DM Manuales</span>
              <span
              ...
              >
                {project.manualDmRolls ? '🎲 Manual' : '⚡ Automático'}
              </span>
            </div>
            <p className="text-xs text-[var(--text-secondary)] font-lora m-0 max-w-2xl leading-relaxed">
              {project.manualDmRolls
                ? 'Modo Manual Activo: La IA jamás resolverá tiradas de PNJs u oposición en secreto. Detendrá la narración y te pedirá que tires tú mismo para garantizar total transparencia y evitar que el Narrador ajuste resultados por la trama.'
                : 'Modo Automático: La IA resuelve en secreto las tiradas de los PNJs y adversarios con dados reales de fondo para no interrumpir el flujo narrativo.'}
            </p>
          </div>
          <button
            onClick={() => {
            ...
            className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-hidden ${
              project.manualDmRolls ? 'bg-[var(--accent)]' : 'bg-stone-300 dark:bg-stone-700'
            }`}
            role="switch"
            aria-checked={Boolean(project.manualDmRolls)}
            title="Alternar entre tiradas de PNJs manuales y automáticas"
          >
            <span
