...
export const StatusView: React.FC<StatusViewProps> = ({
  ...
  const [filtroHilos, setFiltroHilos] = useState<
    "todos" | "activos" | "temporales" | "completados"
  >("activos");
  ...
  const hilosPendientes = threads.filter(
    (h) => h.status === "pending" || !h.status,
  );
  const hilosCumplidos = threads.filter(
    (h) => h.status === "fired" || h.status === "cancelled",
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 md:pb-4 border-b border-[var(--glass-border)]">
        <div>
          ...
          <p className="text-xs sm:text-sm text-[var(--text-secondary)] m-0 mt-1">
            Situación presente de la compañía, consecuencias temporales y
            objetivos gestionados por la narración.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-2.5 pb-2.5 border-b border-[var(--glass-border)]">
          <div className="space-y-0.5">
            ...
            <p className="text-xs text-[var(--text-secondary)] m-0">
              Dónde están ahora mismo, qué peligros inmediatos enfrentan y con
        ...
        {memory.current_status ? (
          ...
          <div className="p-6 rounded-lg border border-dashed border-[var(--glass-border)] bg-[var(--surface-soft)]/20 text-center space-y-1">
            <p className="text-xs sm:text-sm text-[var(--text-secondary)] m-0">
              No hay estado registrado aún. Aparecerá automáticamente según
              juegues o pulses «Sincronizar con IA».
            </p>
          </div>
        )}
      </section>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2.5 border-b border-[var(--glass-border)]">
          <div className="space-y-1">
            ...
            <p className="text-xs text-[var(--text-secondary)] m-0 leading-relaxed">
              Consecuencias con fecha prevista: el mundo actuando por su cuenta.
              El Narrador los escribe según juegas cuando queda algo pendiente
              en el tiempo.
            </p>
          </div>
        </div>
        ...
        {hilosPendientes.length === 0 ? (
          <div className="p-6 rounded-xl border border-dashed border-[var(--glass-border)] text-center">
            <p className="text-sm text-[var(--text-secondary)] italic m-0">
              Nada programado todavía. Aparecerán según juegues y se fijen
            ...
            <ul className="mt-2 space-y-1 pl-4 list-disc">
              {hilosCumplidos.map((h) => (
                <li key={h.id}>
                  ...
                  {h.status === "fired" ? "Cumplido" : "Cancelado"})
                </li>
              ))}
            </ul>
          </details>
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-cinzel font-bold text-sm sm:text-base text-[var(--text-primary)] flex items-center gap-2">
                ...
                <span>
                  MISIONES Y OBJETIVOS DE LA CAMPAÑA ({quests.length})
                </span>
              </span>
            </div>
            <p className="text-xs text-[var(--text-secondary)] m-0 mt-0.5">
              Misiones en curso y objetivos registrados por la narración del
              juego.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
            {[
              ...
              {
                id: "temporales",
                label: "Con Fecha / Plazo",
                count: conteoTemporales,
              },
              {
                id: "completados",
                label: "Completadas",
                count: conteoCompletados,
              },
        ...
        {questsFiltradas.length === 0 ? (
          <div className="p-8 rounded-xl border border-dashed border-[var(--glass-border)] text-center space-y-2">
            ...
            <p className="text-xs text-[var(--text-secondary)] opacity-80 m-0">
              El Narrador las registrará y actualizará según se desarrollen las
              ...
              return (
                ...
                >
                  <div className="space-y-2">
                    ...
                    {/* Título */}
                    <h3
                      className={`font-cinzel font-bold text-sm sm:text-base m-0 ${isDone ? "line-through opacity-80" : "text-[var(--text-primary)]"}`}
                    >
                      {q.title}
                    </h3>
                    ...
                    {q.origin && (
                      <div className="text-[11px] text-[var(--text-secondary)] italic">
                        Origen:{" "}
                        <span className="font-semibold">{q.origin}</span>
                      </div>
                    )}
                    ...
                    {q.objective && (
                      <div className="text-xs text-[var(--text-primary)] leading-relaxed">
                        <strong className="font-cinzel text-[11px] text-[var(--accent)]">
                          Objetivo:{" "}
                        </strong>
                        {q.objective}
                      </div>
                    ...
                    {q.progress && (
                      <div className="text-xs text-[var(--text-secondary)] bg-[var(--bg-color)]/50 p-2 rounded-md border border-[var(--glass-border)] leading-relaxed">
                        <strong className="font-cinzel text-[10px] text-[var(--text-primary)]">
                          Avances:{" "}
                        </strong>
                        {q.progress}
                      </div>
                    ...
                    {relatedThread && (
                      <div className="flex items-center gap-1.5 text-[11px] font-cinzel text-amber-800 dark:text-amber-300 bg-amber-500/10 px-2 py-1 rounded border border-amber-500/20">
                        ...
                        <span>
                          Vence en la cronología:{" "}
                          <strong>{relatedThread.dueDate}</strong>
                        </span>
                      </div>
                    )}
                  </div>
      ...
      <section className="bg-[var(--bg-color)]/70 border border-[var(--glass-border)] rounded-xl p-4 sm:p-5 md:p-6 space-y-3.5 shadow-2xs">
        <div className="flex flex-wrap items-start justify-between gap-2.5 pb-2.5 border-b border-[var(--glass-border)]">
          <div className="space-y-0.5">
            ...
            <p className="text-xs text-[var(--text-secondary)] m-0">
              Reglas de casa, secretos del máster y revelaciones futuras que el
              Narrador respeta estrictamente.
            </p>
          </div>
        ...
        {memory.manual_notes ? (
          mostrarNotasNarrador ? (
            ...
            >
              ...
              <p className="text-xs text-[var(--text-secondary)] opacity-80 m-0">
                Ocultos para evitar spoilers durante la sesión de juego. Haz
                clic para revelar.
              </p>
            </div>
          )
          ...
          <div className="p-6 rounded-lg border border-dashed border-[var(--glass-border)] bg-[var(--surface-soft)]/20 text-center space-y-1">
            <p className="text-xs sm:text-sm text-[var(--text-secondary)] m-0">
              Sin notas por ahora. El Narrador las irá registrando o
              sincronizando a partir de la historia.
            </p>
          </div>
        )}
      </section>
