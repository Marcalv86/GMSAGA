...
export function calendarioValido(
  cal: CalendarConfig | undefined,
): cal is CalendarConfig {
  return Boolean(
    cal &&
...
export function desdeDiaAbsoluto(
  cal: CalendarConfig,
  abs: number,
  minuto = 0,
): CampaignDate {
...
export function fechaLegible(cal: CalendarConfig, fecha: CampaignDate): string {
  ...
  const slot =
    slots[Math.min(Math.max(0, fecha.dayOfYear - 1), slots.length - 1)];
  const sufijo = cal.yearSuffix ? ` ${cal.yearSuffix}` : "";
  if (!slot) return `año ${fecha.year}${sufijo}`;
  if (slot.kind === "festival")
    return `${slot.festivalName} de ${fecha.year}${sufijo}`;
...
export function fechaCompacta(
  ...
  const dia =
    slot.kind === "festival"
      ? slot.festivalName
      : `${slot.day} ${cal.months[slot.monthIndex]?.name || ""}`;
 ...
 */
export function leerAvanceDeTiempo(texto: string): {
  minutos: number;
  encontrado: boolean;
} {
  ...
  while ((m = AGENDA_RE.exec(texto)) !== null) {
    ...
    for (const parte of partes) {
      ...
      if (campo === "lugar") entrada.lugar = valor;
        ...
        if (/noticia|prensa|bando|gaceta|pregonero/i.test(valNorm))
          entrada.tipo = "noticia";
        else if (/rumor|murmullo|taberna/i.test(valNorm))
          entrada.tipo = "rumor";
        else if (/inconscien|coma|convalecen|letargo|herido/i.test(valNorm))
          entrada.tipo = "inconsciencia";
        else if (/salto|elipsis/i.test(valNorm))
    ...
    if (!entrada.tipo) {
      const textoCompleto =
        `${entrada.hito || ""} ${entrada.resumen}`.toLowerCase();
      if (
        /noticia|pregonero|bando municipal|gaceta|tabl[oó]n de anuncios|comunicado|noticias/i.test(
          textoCompleto,
        )
      ) {
        entrada.tipo = "noticia";
      } else if (
        /rumor|se dice en|se comenta|habladur[ií]as|murmullos en la taberna/i.test(
          textoCompleto,
        )
      ) {
        entrada.tipo = "rumor";
      } else if (
        /inconscien|en coma|desmayad|recuper[aá]ndose de las heridas|convalecen|enfermer[ií]a|despert[oó] tras/i.test(
          textoCompleto,
        )
      ) {
        entrada.tipo = "inconsciencia";
      } else if (
        /salto temporal|pasaron los d[ií]as|semanas despu[eé]s/i.test(
          textoCompleto,
        )
      ) {
        entrada.tipo = "salto_temporal";
...
export function obtenerInfoRelacion(texto?: string): RelacionInfo {
  ...
  if (
    /romance|amor|enamor|cortej|declaraci|insinuaci|pasion|sexual|emocional|pareja|beso|amante|interes romantico|atracci|quimica/i.test(
      t,
    )
  ) {
    return {
  ...
  // Enemistad / Odio
  if (
    /enemig|antagonista|rencor|odio|hostil|venganza|enemistad|amenaza/i.test(t)
  ) {
    return {
      icono: "💀",
      tipo: "enemistad",
      label: "Enemistad",
      badgeClass:
        "bg-red-100 text-red-900 border-red-300 dark:bg-red-950/60 dark:text-red-300 dark:border-red-700/60",
    };
  }
  ...
  if (/traici|ruptura|desamor|desengano|abandono/i.test(t)) {
    return {
      ...
      badgeClass:
        "bg-purple-100 text-purple-900 border-purple-300 dark:bg-purple-950/60 dark:text-purple-300 dark:border-purple-700/60",
    };
  }
  ...
  if (/desconfian|tension|suspicaz|recelo|vigilancia|frialdad/i.test(t)) {
    return {
      ...
      label: "Desconfianza",
      badgeClass:
        "bg-orange-100 text-orange-900 border-orange-300 dark:bg-orange-950/60 dark:text-orange-300 dark:border-orange-700/60",
    };
  }
 ...
 */
export function estacionDelDia(
  cal: CalendarConfig,
  dayOfYear: number,
): Estacion {
  const slots = yearLayout(cal);
...
export function leerHilos(texto: string): HiloLeido[] {
  ...
  while ((m = HILO_RE.exec(texto)) !== null) {
    ...
    const dias =
      minutos > 0 ? Math.max(1, Math.round(minutos / MINUTOS_POR_DIA)) : 0;
    if (!dias) continue;
...
export function hilosQueVencen(
  hilos: ScheduledThread[],
  diaAbs: number,
): ScheduledThread[] {
  return (hilos || []).filter(
    (h) => h.status === "pending" && h.dueAbsDay <= diaAbs,
  );
}
...
export function diasDelMes(
  ...
  slots.forEach((s, i) => {
    ...
    out.push({
      dayOfYear: i + 1,
      etiqueta:
        s.kind === "festival" ? s.festivalName || "Festival" : String(s.day),
...
export function leerVinculos(texto: string): VinculoLeido[] {
  ...
  if (/V[IÍ]NCULO/i.test(texto)) {
    ...
    while ((m = VINCULO_RE.exec(texto)) !== null) {
      ...
      for (const parte of partes) {
        ...
        else if (campo === "oculta" || campo === "calla" || campo === "piensa")
          v.oculta = valor;
        else if (
          campo === "grado" ||
          campo === "vinculo" ||
  ...
  if (/AFINIDAD/i.test(texto)) {
    ...
    while ((am = AFINIDAD_TAG_RE.exec(texto)) !== null) {
      ...
      let existing = out.find((item) =>
        coincidenNombresNpc(item.nombre, nombre),
      );
      if (!existing) {
        existing = { nombre };
        out.push(existing);
      ...
      for (const parte of partes) {
        ...
        if (campo === "atr" || campo === "atraccion")
          existing.atr = Math.max(0, Math.min(20, num));
        else if (campo === "vin" || campo === "afecto")
          existing.vin = Math.max(0, Math.min(20, num));
        else if (campo === "con" || campo === "confianza")
          existing.con = Math.max(0, Math.min(20, num));
      }
    }
  }
