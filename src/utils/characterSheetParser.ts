...
export const DND_SKILL_DEFINITIONS: Array<{
...
}> = [
  {
    ...
    statKey: "dex",
    statShort: "DES",
    aliases: ["acrobacias", "acrobatics", "acrobacia"],
  },
  {
    name: "Arcanos",
    statKey: "int",
    statShort: "INT",
    aliases: ["arcanos", "arcana", "arcano", "conocimiento arcano"],
  },
  {
    name: "Atletismo",
    statKey: "str",
  ...
  },
  {
    ...
    statKey: "cha",
    statShort: "CAR",
    aliases: ["engaño", "engano", "deception"],
  },
  {
    name: "Historia",
    statKey: "int",
  ...
  },
  {
    ...
    statKey: "cha",
    statShort: "CAR",
    aliases: ["interpretación", "interpretacion", "performance", "actuación"],
  },
  {
    name: "Intimidación",
    statKey: "cha",
    statShort: "CAR",
    aliases: ["intimidación", "intimidacion", "intimidation"],
  },
  {
    name: "Intuición",
    statKey: "wis",
    statShort: "SAB",
    aliases: ["intuición", "intuicion", "insight", "perspicacia"],
  },
  {
    name: "Investigación",
    statKey: "int",
    statShort: "INT",
    aliases: ["investigación", "investigacion", "investigation"],
  },
  {
    name: "Juego de Manos",
    statKey: "dex",
    statShort: "DES",
    aliases: ["juego de manos", "sleight of hand", "prestidigitacion"],
  },
  {
    name: "Medicina",
    statKey: "wis",
    statShort: "SAB",
    aliases: ["medicina", "medicine"],
  },
  {
    name: "Naturaleza",
    statKey: "int",
    statShort: "INT",
    aliases: ["naturaleza", "nature"],
  },
  {
    name: "Percepción",
    statKey: "wis",
    statShort: "SAB",
    aliases: ["percepción", "percepcion", "perception"],
  },
  {
    name: "Persuasión",
    statKey: "cha",
    statShort: "CAR",
    aliases: ["persuasión", "persuasion"],
  },
  {
    name: "Religión",
    statKey: "int",
    statShort: "INT",
    aliases: ["religión", "religion"],
  },
  {
    name: "Sigilo",
    statKey: "dex",
    statShort: "DES",
    aliases: ["sigilo", "stealth"],
  },
  {
    name: "Supervivencia",
    statKey: "wis",
    statShort: "SAB",
    aliases: ["supervivencia", "survival"],
  },
  {
    name: "Trato con Animales",
    statKey: "wis",
    statShort: "SAB",
    aliases: ["trato con animales", "animal handling", "manejo de animales"],
  },
];
 ...
 */
export function refineAndDeduplicateInventory(
  ...
  const filtered = rawItems.filter((item) => {
    ...
    if (
      /^(?:competencias?|idiomas?|armaduras?\s+ligeras?|armas?\s+simples?|herramientas?\s+de|salvaciones?\s+con|rasgos?\s+de|dotes?)\b/i.test(
        norm,
      ) ||
  ...
  const hasNamedStaff = filtered.some(
    (item) =>
      /bast[oó]n\s+de\s+[a-záéíóúñ]+/i.test(item.name) &&
      !/bast[oó]n\s+(?:de\s+viaje|com[uú]n|b[aá]sico|de\s+madera)/i.test(
        item.name,
      ),
  );

  const hasNamedSword = filtered.some((item) =>
    /(?:espada|espad[oó]n|daga|arco|hacha)\s+(?:de\s+|del\s+|lunar|solar|m[aá]gic)/i.test(
      item.name,
    ),
  );
  ...
  let deduped = filtered.filter((item) => {
    ...
    if (
      hasNamedStaff &&
      /^(?:baston(?:\s+(?:de\s+viaje|comun|basico|de\s+madera))?|baston\s+de\s+viajero)$/i.test(
        norm,
      )
    ) {
      return false;
    ...
    if (
      hasNamedSword &&
      /^(?:espada\s+comun|espada\s+corta|espada\s+larga|daga\s+comun)$/i.test(
        norm,
      )
    ) {
      return false;
  ...
  deduped = deduped.map((item) => {
    ...
    const isOracularOrRelic =
      /fionn|ogham|oraculo|reliquia|foco\s+druidico|foco\s+arcano|simbolo\s+sagrado|tablilla/i.test(
        norm,
    ...
    if (isOracularOrRelic) {
      return {
        ...
        damageOrAc:
          item.damageOrAc &&
          !item.damageOrAc.includes("+2 CA") &&
          !item.damageOrAc.includes("CA")
            ? item.damageOrAc
            : "Foco ritual / Oráculo",
      };
    }
  ...
  if (rawSourceText) {
    ...
    const sourceMentionsShieldAsItem =
      /(?:inventario|equipo|posesiones)[\s\S]*?\b(?:escudo\s+de\s+madera|escudo\s+de\s+combate|escudo\s+de\s+acero|escudo\s+de\s+cuero)\b/i.test(
        normSource,
      );
  ...
  const isWieldingTwoHanded = deduped.some((item) => {
    ...
    const desc =
      `${item.name} ${item.damageOrAc || ""} ${item.description || ""}`.toLowerCase();
    return (
...
export function validateCharacterEquipment(
  ...
  const inventory = refineAndDeduplicateInventory(
    pc.inventory || [],
    pc.sheetText,
  );
 ...
 */
export function parseDndSheetText(
  rawText: string,
  existingPc?: Partial<PlayerCharacter>,
  ...
  for (let i = 0; i < lines.length - 2; i++) {
    ...
    if (
      ...
      if (headers.length >= 6 && vals.length >= 6) {
        const hasDndHeader = headers.some((h) =>
          [
            "FUE",
            "STR",
            "DES",
  ...
  const hdMatch = text.match(
    /(?:Dados?\s+de\s+Golpe|Hit\s+Dice)\s*[:·*]*\s*([0-9dD+\-\s]+)/i,
  );
  ...
  const caMatch = text.match(
    /(?:\*\*CA\*\*|\bCA\b|\bAC\b|Clase\s+de\s+Armadura|Armor\s+Class)\s*[:·*]*\s*(\d+)/i,
  );
  if (caMatch) {
  ...
  const compMatch = text.match(
    /(?:\*\*Comp\.\*\*|\bComp\.\b|\bCompetencia\b|Bono\s+de\s+Comp\w*|Proficiency(?:\s+Bonus)?)\s*[:·*]*\s*([+\-]?\d+)/i,
  );
  if (compMatch) {
  ...
  const salvMatch = text.match(
    /(?:Salvaciones(?:\s+con\s+competencia)?|Tiradas\s+de\s+salvaci[oó]n|Saving\s+Throws)\s*[:*]*\s*([^\n|.]+)/i,
  );
  if (salvMatch) {
    ...
    if (found.length)
      result.savingThrowProficiencies = Array.from(new Set(found));
  }

  // 9. HABILIDADES
  const expertMatch = text.match(
    /Habilidades\s+expertas\s*[:*]*\s*([^\n.]+?)(?:\.\s*Competentes|\.|$)/i,
  );
  ...
  const compSkillsMatch = text.match(
    /(?:Competentes|Habilidades\s+competentes|Competencias\s+en\s+habilidades|Habilidades|Skills)\s*[:*]*\s*([^\n.]+?)(?:\.\s*Dotes|\.|$)/i,
  );
  ...
  const magicMatch = text.match(
    /(?:Magia|Spellcasting|Aptitud\s+M[aá]gica|Lanzamiento\s+de\s+conjuros)\s*[:*]*\s*([^\n.]+)/i,
  );
  if (magicMatch) {
    const mStr = magicMatch[1];
    const abilityMatch = mStr.match(
      /\b(SAB|INT|CAR|FUE|DES|CON|WIS|CHA|STR|DEX)\b/i,
    );
    ...
    if (abilityMatch) {
      ...
      else if (abUpper === "SAB" || abUpper === "WIS")
    ...
    result.spellcasting = {
      ...
      attackBonus: atkMatch
        ? parseInt(atkMatch[1].replace("+", ""), 10)
        : undefined,
    };
  }
  ...
  const spMatch = text.match(
    /(\d+)\s*(?:PP|SP|plata|silver|piezas?\s+de\s+plata)/i,
  );
  if (spMatch) currencies.sp = parseInt(spMatch[1], 10);

  const cpMatch = text.match(
    /(\d+)\s*(?:PC|CP|cobre|copper|piezas?\s+de\s+cobre)/i,
  );
  ...
  const ppMatch = text.match(
    /(\d+)\s*(?:PT|PP|platino|platinum|piezas?\s+de\s+platino)/i,
  );
  ...
  const inventorySection = text.match(
    /(?:###?\s*(?:Inventario|Equipo|Objetos|Armas|Posesiones|Equipment|Items)[\s\S]*?)(?:###|$)/i,
  );
  ...
  if (inventorySection) {
    ...
    lines.forEach((line, idx) => {
      ...
      if (
        ...
        if (
          /^(?:nombre|clase|raza|nivel|pg|ca|vel|inic|fue|des|con|int|sab|car)\b/i.test(
            clean,
          )
        )
          return;
        ...
        if (
          /espada|hacha|maza|arco|bast[oó]n|daga|lanza|martillo|greataxe|sword|bow/i.test(
            clean,
          )
        )
          category = "weapon";
        else if (/armadura|escudo|cota|cuero|shield|armor/i.test(clean))
          category = "armor";
        else if (/poci[oó]n|elixir|ung[uü]ento|potion/i.test(clean))
          category = "potion";
        else if (/pergamino|scroll|tomo|grimorio/i.test(clean))
          category = "scroll";
        else if (/anillo|amuleto|vara|foco|varita|magic/i.test(clean))
          category = "magic";
 ...
 */
export function ensureValidPlayerCharacter(
  pc?: PlayerCharacter,
): PlayerCharacter {
