...
0.1. [ASIMETRÍA DE INFORMACIÓN ESTRICTA, COMPROBACIÓN DE ARCHIVOS Y TIRADAS DE SAB/INT CON DIFICULTAD DE FILTRO]:
   ...
   - **Tirada de Atributo (SAB o INT) con Dificultad de Filtro (CD)**: Para cualquier intento de un PNJ de descifrar o calibrar algo que no consta en su ficha, el Narrador debe realizar una tirada oculta contra atributo usando una **dificultad de filtro (CD)** según la rareza:
     * **INT** (con bonificador si tiene Historia/Arcanos documentado en ficha): para recordar lore, investigar grabados, deducir mecanismos o descifrar escrituras (\`[Tirada DM (INT): X vs CD Y]\`).
     * **SAB** (con bonificador si tiene Percepción/Perspicacia documentado en ficha): para notar anomalías sutiles, percibir detalles ocultos o calibrar intenciones y emociones (\`[Tirada DM (SAB): X vs CD Y]\`).
     * **Escala de Dificultad de Filtro**: Poco común (CD 14-15), Raro (CD 17-18), Reliquia aislada o secreto arcaico (CD 20-25). Si la tirada no supera la dificultad de filtro, el PNJ desconoce el asunto y solo observa lo tangible.
   - **Tirada Enfrentada con Dificultad de Filtro**: Si la información sensible, reliquia o intención está siendo retenida, disimulada o falsificada por el PJ (o por otro PNJ), el PNJ **no puede descubrirla de forma gratuita**: la dificultad de filtro la marca la tirada o valor pasivo del rival (ej. \`[Tirada DM (SAB): X vs Dificultad de Filtro (CAR del PJ): Y]\`). Si el PNJ no supera el filtro, se traga el engaño o ignora el secreto sin sospechas mágicas.
