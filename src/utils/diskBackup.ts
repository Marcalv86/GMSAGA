...
export async function chooseBackupFolder(): Promise<{
  ok: boolean;
  name?: string;
  error?: string;
}> {
  if (isRunningInIframe()) {
    return {
      ok: false,
      error:
        'El navegador bloquea la selección directa de carpetas en disco dentro de un marco integrado (iframe). Abre la aplicación en una pestaña independiente para activar el guardado en disco, o utiliza la pestaña "Copias JSON" para descargar tus campañas.',
    };
  }
  if (!isDiskBackupSupported()) {
    return {
      ok: false,
      error:
        "Este navegador no admite la selección de carpetas en disco (File System Access API).",
    };
  }
  try {
    ...
    if (
      ...
      (err?.message && err.message.toLowerCase().includes("sub frame"))
    ) {
      return {
        ok: false,
        error:
          "El navegador bloquea el acceso a carpetas locales dentro de un marco integrado (iframe). Abre la app en una pestaña nueva o descarga una copia JSON.",
      };
    }
...
export async function writeCampaignToDisk(
  ...
  written: boolean;
  fileName?: string;
  reason?: "no-folder" | "no-permission" | "error";
}> {
  const handle = await readHandle();
...
export async function deleteCampaignFileFromDisk(
  ...
  try {
    const state = await (handle as any).queryPermission({ mode: "readwrite" });
    if (state !== "granted") {
      const req = await (handle as any).requestPermission({
        mode: "readwrite",
      });
      if (req !== "granted")
        return { ok: false, error: "Permiso de escritura no concedido." };
    }
    ...
    if (typeof (handle as any).removeEntry === "function") {
      ...
      return {
        ok: false,
        error:
          "Tu navegador no soporta la eliminación directa de archivos en disco.",
      };
    }
  } catch (err: any) {
    ...
    return {
      ok: false,
      error: err?.message || "No se pudo eliminar el archivo de la carpeta.",
    };
  }
}
...
export async function listCampaignFilesFromDisk(): Promise<{
  ...
  const handle = await readHandle();
  if (!handle)
    return { ok: false, files: [], error: "No hay carpeta vinculada." };
  ...
  try {
    ...
    return {
      ...
      files: [],
      error:
        err?.message || "No se pudieron listar los archivos de la carpeta.",
    };
  }
}
