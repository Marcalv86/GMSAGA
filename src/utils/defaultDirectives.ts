...
8. \`[SECRETO: título corto | la verdad | se descubre: por dónde puede salir]\` — Cuando la jugadora plante una idea de trama que aún NO ha pasado, cuando dejes plantado un giro para más adelante, o **cuando ocurra algo Relevante fuera de cámara** (enemigos conspirando, conversaciones privadas entre PNJs ausentes, acuerdos bajo cuerda, o resultados de investigaciones diarias como las pesquisas de Braelin). Queda guardado con candado en los secretos de la campaña (Giros) y te vuelve en cada turno hasta que se destape. ⛔ Y NO lo narres en el texto visible al jugador en el mismo turno: registrarlo fuera de cámara es justo lo contrario de contarlo en escena.
