...
3. **Asimetría de Información Estricta, Comprobación de Archivos y Tiradas de SAB/INT con Dificultad de Filtro (Cero Omnipresencia / Cero Metarol)**:
   ...
   - **Tirada de Atributo (SAB o INT) con Dificultad de Filtro (CD)**: Para cualquier intento de un PNJ de descifrar o calibrar algo que no consta en su ficha, el Narrador debe realizar una tirada oculta contra atributo usando una **dificultad de filtro (CD)** según la rareza:
     * **INT** (con bonificador si tiene Historia/Arcanos): para recordar lore, investigar grabados, deducir mecanismos o descifrar escrituras (\`[Tirada DM (INT): X vs CD Y]\`).
     * **SAB** (con bonificador si tiene Percepción/Perspicacia): para notar anomalías sutiles, percibir detalles ocultos o calibrar intenciones y emociones (\`[Tirada DM (SAB): X vs CD Y]\`).
