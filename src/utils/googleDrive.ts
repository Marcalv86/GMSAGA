...
export async function getOrCreateAppFolder(
  ...
  if (!searchRes.ok) {
    ...
    throw new Error(
      `Error al consultar Google Drive: ${searchRes.status} ${errorText}`,
    );
  }
  ...
  const createRes = await fetch(
    "https://www.googleapis.com/drive/v3/files?fields=id,name,webViewLink",
    {
      ...
      body: JSON.stringify({
        ...
        description:
          "Carpeta de copias de seguridad de campañas y fichas de Saga Viviente (GM Studio).",
      }),
    },
  );
  ...
  if (!createRes.ok) {
    ...
    throw new Error(
      `No se pudo crear la carpeta en Google Drive: ${errorText}`,
    );
  }
...
export async function saveCampaignsToGoogleDrive(
  ...
  if (!uploadRes.ok) {
    ...
    throw new Error(
      `Error al subir a Google Drive: ${uploadRes.status} ${errorText}`,
    );
  }
...
export async function listDriveBackups(
  accessToken: string,
): Promise<{ folder: DriveFolderInfo; files: DriveFileInfo[] }> {
  const folder = await getOrCreateAppFolder(accessToken);
...
export async function downloadCampaignsFromGoogleDrive(
  ...
  throw new Error(
    "El archivo descargado de Google Drive no contiene un formato de campañas reconocido.",
  );
}
