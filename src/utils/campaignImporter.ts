 ...
 */
export async function readRawFileText(
  file: File,
): Promise<{ text: string; isPdf: boolean; isJson: boolean }> {
  const isPdf =
...
export async function importCampaignWithGemini(
  ...
  if (!cleaned || cleaned.length < 20) {
    throw new Error(
      "El texto proporcionado está vacío o es demasiado corto para extraer una campaña.",
    );
  }

  // Limitamos el texto enviado a unos 120k caracteres si es masivo para respuesta óptima
  const truncatedText =
    cleaned.length > 130000
      ? cleaned.slice(0, 130000) + "\n\n[...texto truncado por longitud...]"
      : cleaned;
...
export function importCampaignLocalFallback(
  ...
  const campaignName =
    preferredTitle || extractTitleFromText(lines) || "Campaña Importada";
  ...
  const protagonist: PlayerCharacter = {
    ...
    attributes: parsedSheet.attributes || {
      str: 10,
      dex: 10,
      con: 10,
      int: 10,
    ...
    currencies: parsedSheet.currencies || {
      cp: 0,
      sp: 0,
      ep: 0,
      gp: 10,
...
function formatParsedDataToCampaign(
  ...
  const npcs: NPC[] = Array.isArray(parsed.npcs)
    ? parsed.npcs.map((n: any, i: number) => ({
        ...
        atr:
          typeof n.atr === "number"
            ? Math.max(0, Math.min(20, Math.round(n.atr)))
            : undefined,
        vin:
          typeof n.vin === "number"
            ? Math.max(0, Math.min(20, Math.round(n.vin)))
            : undefined,
        con:
          typeof n.con === "number"
            ? Math.max(0, Math.min(20, Math.round(n.con)))
            : undefined,
        recurrente: Boolean(
          n.vinculo ||
          n.aparenta ||
          n.oculta ||
          (n.vin !== undefined && n.vin >= 4),
        ),
      }))
    : [];
  ...
  const memory: Memory = {
    ...
    manual_notes:
      parsed.manualNotes ||
      `Documento importado (${rawText.length} caracteres).`,
  ...
  const project: Project = {
    id: projectId,
    name,
    instructions: parsed.directives
  ...
  for (const line of lines.slice(0, 15)) {
    ...
    if (/^(campaña|tomo|aventura|crónica|partida)\s*:\s*(.+)/i.test(trimmed)) {
      const match = trimmed.match(
        /^(campaña|tomo|aventura|crónica|partida)\s*:\s*(.+)/i,
      );
...
function extractChatsDeterministically(lines: string[]): {
  chapters: Chat[];
  messagesCount: number;
} {
  const chapters: Chat[] = [];
  ...
  for (const line of lines) {
    ...
    if (
      /^#+\s+(Capítulo|Sesión|Parte|Chapter|Acto)\s+/i.test(trimmed) ||
      /^===\s+(.+)\s+===/.test(trimmed)
    ) {
      flushChapter();
      currentChapterName = trimmed
        .replace(/^[#=\s]+/, "")
        .replace(/[#=\s]+$/, "")
        .trim();
      continue;
    ...
    const userMatch = trimmed.match(
      /^(Tú|User|Usuario|Jugador|Player|Prompt)\s*:\s*(.*)/i,
    );
    const modelMatch = trimmed.match(
      /^(Gemini|Model|Narrador|DM|GM|Master|Assistant|IA|Respuesta)\s*:\s*(.*)/i,
    );
  ...
  for (const line of lines) {
    ...
    if (m) {
      ...
      if (
        name.length > 1 &&
        name.length < 40 &&
        !npcs.some((n) => n.name.toLowerCase() === name.toLowerCase())
      ) {
        npcs.push({
...
function extractQuestsDeterministically(lines: string[]): Quest[] {
  ...
  for (const line of lines) {
    const m = line
      .trim()
      .match(
        /^[*-]\s+\[?\s*(?:Misión|Quest|Encargo|Objetivo)\s*\]?\s*[:—–-]?\s*\*\*?([^*]+)\*\*?\s*[:—–-]?\s*(.*)/i,
      );
    if (m) {
      quests.push({
...
function extractLocationsDeterministically(lines: string[]): Location[] {
  ...
  for (const line of lines) {
    const m = line
      .trim()
      .match(
        /^[*-]\s+\[?\s*(?:Lugar|Ubicación|Location|Ciudad|Posada|Región)\s*\]?\s*[:—–-]?\s*\*\*?([^*]+)\*\*?\s*[:—–-]?\s*(.*)/i,
      );
    if (m) {
      locs.push({
